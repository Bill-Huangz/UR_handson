import argparse
from experiments.common.setup_experiment import flush_logs, get_value_logger #, setup_experiment
...
def main():
    ...
    log_dir = args.data_folder + 'flow_model/joint/'
    logger = get_value_logger(log_dir)
    seed_everything(args.seed)

    state = np.load(args.data_folder + "state.npy")
    action = np.load(args.data_folder + "action.npy")

    joint_data = np.concatenate((state, action), axis=1)
    dim = joint_data.shape[1]
    ...
    # Load dataset
    data_orign = th.from_numpy(joint_data).double().to(args.device)
